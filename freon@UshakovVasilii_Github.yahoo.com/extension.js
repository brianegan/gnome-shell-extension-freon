...
function _makeLogFunction(prefix) {
    return (msg) => {
        ...
        let [_, func, file, line] = regex.exec(trace);
        file = GLib.path_get_basename(file);

        let hdr = [file, func, line].filter((k) => k).join(":");

        GLib.log_structured("freon", GLib.LogLevelFlags.LEVEL_MESSAGE, {
...
const FreonMenuButton = GObject.registerClass(
    class Freon_FreonMenuButton extends PanelMenu.Button {
        _init() {
            ...
            let temperatureIcon = Gio.icon_new_for_string(
                Me.path +
                    "/icons/material-icons/material-temperature-symbolic.svg"
            );
            this._sensorIcons = {
                temperature: temperatureIcon,
                "temperature-average": temperatureIcon,
                "temperature-maximum": temperatureIcon,
                "gpu-temperature": Gio.icon_new_for_string(
                    Me.path +
                        "/icons/material-icons/material-gpu-temperature-symbolic.svg"
                ),
                "drive-temperature": Gio.icon_new_for_string(
                    "drive-harddisk-symbolic"
                ),
                voltage: Gio.icon_new_for_string(
                    Me.path + "/icons/freon-voltage-symbolic.svg"
                ),
                fan: Gio.icon_new_for_string(
                    Me.path + "/icons/freon-fan-symbolic.svg"
                ),
            };
            ...
            this._addSettingChangedSignal(
                "unit",
                this._querySensors.bind(this)
            );
            this._addSettingChangedSignal(
                "show-degrees-on-panel",
                this._updateUI.bind(this)
            );
            this._addSettingChangedSignal(
                "show-icon-on-panel",
                this._showIconOnPanelChanged.bind(this)
            );
            this._addSettingChangedSignal(
                "hot-sensors",
                this._querySensors.bind(this)
            ...
            this._addSettingChangedSignal(
                "drive-utility",
                this._driveUtilityChanged.bind(this)
            );
            this._addSettingChangedSignal(
                "gpu-utility",
                this._gpuUtilityChanged.bind(this)
            );
            this._addSettingChangedSignal(
                "position-in-panel",
        ...
        _showIconOnPanelChanged() {
            ...
            if (showIcon) {
                ...
                for (let k in this._hotIcons) this._hotIcons[k].destroy();
                this._hotIcons = {};
            }
            for (let l in this._hotLabels)
                this._hotLabels[l].set_style_class_name(
                    showIcon
                        ? "freon-panel-icon-label"
                        : "freon-panel-no-icon-label"
                );
        ...
        _updateDisplay() {
            ...
            sensorsTempInfo.sort(function (a, b) {
                return a.label.localeCompare(b.label);
            });
            driveTempInfo.sort(function (a, b) {
                return a.label.localeCompare(b.label);
            });
            fanInfo.sort(function (a, b) {
                return a.label.localeCompare(b.label);
            });
            voltageInfo.sort(function (a, b) {
                return a.label.localeCompare(b.label);
            });

            let tempInfo = gpuTempInfo
                .concat(sensorsTempInfo)
                .concat(driveTempInfo);
            ...
            if (tempInfo.length > 0) {
                ...
                if (
                    sensorsTempInfo.length > 0 &&
                    this._settings.get_boolean("group-temperature")
                ) {
                    sum = 0;
                    for (let i of sensorsTempInfo) {
                        sum += i.temp;
        ...
        _appendStaticMenuItems() {
            ...
            if (this._settings.get_boolean("show-wiki")) {
                let wiki = new PopupMenu.PopupBaseMenuItem();
                wiki.actor.add_child(
                    new St.Label({
                        text: _("Go to the Freon wiki"),
                        x_expand: true,
                    })
                );
            ...
            settings.actor.add(new St.Icon({ style_class: 'popup-menu-icon', gicon : new Gio.ThemedIcon({name: 'emblem-system-symbolic'})}));
        ...
        _appendMenuItems(sensors) {
            this._lastSensorsCount = sensors.length;
            this._sensorMenuItems = {};
            let needGroupTemperature = this._settings.get_boolean(
                "group-temperature"
            );
            ...
            if (needGroupVoltage) {
                let i = 0;
                for (let s of sensors) if (s.type == "voltage") i++;
            ...
            for (let s of sensors) {
                if (s.type == "separator") {
                    ...
                    let item = new FreonItem.FreonItem(
                        this._sensorIcons[s.type],
                        key,
                        s.label,
                        s.value,
                    ...
                    item.connect("activate", (self) => {
                        ...
                        if (l) {
                            ...
                            let showIcon = this._settings.get_boolean(
                                "show-icon-on-panel"
                            );
                    ...
                    if (needGroupTemperature && s.type == "temperature") {
                        if (!temperatureGroup) {
                            temperatureGroup = new PopupMenu.PopupSubMenuMenuItem(
                                _("Temperature Sensors"),
                                true
                            );
                            temperatureGroup.icon.gicon = this._sensorIcons[
                                "temperature"
                            ];
                            if (!temperatureGroup.status) {
                                // gnome 3.18 and hight
                                temperatureGroup.status = new St.Label({
                                    style_class: "popup-status-menu-item",
                                    y_expand: true,
                                    y_align: Clutter.ActorAlign.CENTER,
                                });
                                temperatureGroup.actor.insert_child_at_index(
                                    temperatureGroup.status,
                                    4
                                );
                            }
                        ...
                        if (!voltageGroup) {
                            voltageGroup = new PopupMenu.PopupSubMenuMenuItem(
                                _("Voltage"),
                                true
                            );
                            voltageGroup.icon.gicon = this._sensorIcons[
                                "voltage"
                            ];
        ...
        _formatTemp(value) {
            ...
            if (this._settings.get_boolean("show-degrees-on-panel")) {
                return format.format(
                    value,
                    this._settings.get_string("unit") == "fahrenheit"
                        ? "\u00b0F"
                        : "\u00b0C"
                );
            } else {
